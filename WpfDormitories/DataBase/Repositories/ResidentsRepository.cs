...
namespace WpfDormitories.DataBase.Repositories
{
    public class ResidentsRepository : IRepository<IResidentData>
    {
        ...
        public ICollection<IResidentData> Read()
        {
            string query = "SELECT * FROM evictions";
            DataTable dt = DormitorySQLConnection.GetInstance().GetData(query);
            ICollection<IResidentData> result = new Collection<IResidentData>();
                    ...
                    new ResidentData(
                        ...
                        new PersonDocuments(
                            ...
                            new Passport(
                                ...
                                row[7].ToString(), DateOnly.Parse(row[8].ToString()), uint.Parse(row[9].ToString()),
                                uint.Parse(row[10].ToString()), DateOnly.Parse(row[11].ToString()), row[12].ToString())),
                        int.Parse(row[15].ToString()) == 1 ? true : false,
                        DateOnly.Parse(row[16].ToString()),
                        (float)row[17],
                        row[18]?.ToString(),
                        row[19]?.ToString())
                    );
